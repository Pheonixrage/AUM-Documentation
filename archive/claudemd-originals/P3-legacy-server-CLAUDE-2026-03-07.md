...
| Dev server | `/root/aum-server-dev/` port `6006` (manual start) |
...
ssh root@65.109.133.129 "pkill -f orchestrator.py; cd /root/orchestrator && nohup python3 orchestrator.py > orchestrator.log 2>&1 &"
```
...
### Useful commands
```bash
tail -f /root/orchestrator/orchestrator.log    # Orchestrator logs
...
*Last Updated: February 18, 2026 (CloudScript rev 72/28 — Tamas receivedKarma self-deduction, lives bracket fix)*
