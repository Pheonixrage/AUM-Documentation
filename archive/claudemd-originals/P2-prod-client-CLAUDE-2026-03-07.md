...
| CloudScript | Dev revision 132 / Prod revision 59 |
...
| `Assets/Scripts/PlayFab/PlayFabLobbyService.cs` | Lobby system: CloudScript calls, polling, orchestrator allocation, match start |
...
| Date | Fix | Files |
|------|-----|-------|
| **Mar 5** | **Party/Lobby 2v2v2 wrong map size + match type corruption across scene transitions** — Players spawned in wrong positions for Duo_2v4 (2v2v2) because MapLoader loaded MapSize.Small instead of Large. Root cause: `MatchTypeUIScroll.OnDisable()` fires during MainMenu→Map_Hell scene unload, resetting `currentMatchType` from `Duo_2v4` to `Duo_2v2` (Duo toggle default). Latent bug since Jan 22 — never caught because 2v2v2 wasn't tested end-to-end through Party/Lobby. Fix: Added `_activeMatchType` field to PlayFabMatchmaker that locks match type at search start, immune to UI resets. All 11 internal read sites switched from `currentMatchType` to `_activeMatchType`. `ActiveMatchType` public property exposed. Restored in `MapLoader.Start()` (primary — before map size switch), `GameManager.Start()` (safety net), and before `OnMatchFound` fires (3 paths in PlayFabMatchmaker). Reset in `MainEntranceController.OnEnable()` to prevent stale values. Also: duplicate `FindMatchWithParty` guard, party member Players list for team assignment, `_matchSetupInProgress` reset. Server: NAT-safe player counting (`playersByUniqueCode` instead of `playerList`) + `karmaSelectionComplete` reset. | PlayFabMatchmaker.cs, MapLoader.cs, GameManager.cs, MainEntranceController.cs, PlayFabPartyService.cs, Server: GameManager.cs |
| **Mar 4** | **Lobby match rewards disabled + HideRewardsForLobby panel** — Lobby matches (including PvP lobby) should not grant any rewards. MatchEndManager already had gates at lines 617-624 (skips CloudScript persistence) and 1146-1151 (skips UpdatePlayerRewardsFromMatch). But MatchDetailScreen used `LobbyManager.Instance.isInLobby` which is cleared to `false` at PlayFabLobbyService line 630 before match starts — always showed rewards. Fix: Changed to `PlayFabLobbyService.Instance.InLobbyMatch` (stays `true` through match). Early return activates `hideRewardsPanel` ("Reward not received as you were in the lobby") and sets `matchRewardsApplied = true`. Karma selection (GunaScreen) is NOT skipped for lobby matches. | MatchDetailScreen.cs |
| **Mar 4** | **Lobby PvP match accept/cancel popup** — Added lobby-aware handlers for the Match Found popup. (1) `OnMatchFoundCancelled`: if `InLobbyMatch`, calls `ReturnToLobbyAfterMatch()` instead of navigating to main menu. (2) `OnPreConnectFailed`: if `InLobbyMatch`, returns to lobby with NO bot fallback (lobby matches should never fall back to bots). Both properly clean up PreConnect state. | MainEntranceController.cs |
| **Mar 4** | **Lobby PvP match accept not registering (\_matchAcceptHandled stale state)** — Both players click Accept in Match Found popup but PLAYER_READY(0) never sent to server. Server's 12s countdown expires → MATCH_CANCELLED. Root cause: `_matchAcceptHandled` set `true` in `OnMatchFoundAccepted()` but never reset in lobby/PvP match flow. Only reset in `OnCancelMatchSearch` (normal cancel button) and `StartLocalOrLANMatch` (LAN mode). After any successful accept (even if server subsequently cancelled), ALL future Accept clicks silently skipped by `if (_matchAcceptHandled) return;` guard. Secondary issue: `OnPreConnectFailed` hid popup without unsubscribing `OnAccepted`/`OnCancelled` events → duplicate handlers accumulated on repeated match attempts. Fix: (1) Added `_matchAcceptHandled = false` reset in 4 places: `OnLobbyMatchReady`, `OnPlayFabMatchFound`, `OnMatchFoundCancelled`, `OnPreConnectFailed`. (2) Added proper popup event unsubscription in `OnPreConnectFailed` before `Hide()`. Verified safe for normal PvP flow — reset is harmless on first match (default `false`), and correctly resets for repeat matches. | MainEntranceController.cs |
| **Mar 3** | **Deferred avatar rebuild for AVATARSYNC race during packet replay** — First player to join PvP sees wrong opponent character model. During PreConnect packet replay, CREATECHARACTER fires `CreateBotAvatar` with stale wildcard placeholder data (e.g. MantraMuktha when real player is YantraMuktha). The corrective AVATARSYNC arrives while CreateBotAvatar is mid-coroutine (`matchAvatars[i]` still null), so the existing rebuild trigger (`matchAvatars[foundIndex] != null`) silently skips. This extends the Feb 26 AVATARSYNC rebuild fix which only handled the case where AVATARSYNC arrives AFTER the model is fully built. Fix: Added `_pendingRebuildIndices` HashSet deferred rebuild pattern in GameManager. NetworkManager's rebuild trigger now splits into two paths: (1) model already built → immediate `RebuildAvatar()` (existing behavior), (2) model still building → `RequestDeferredRebuild(index)` queues the index. After `matchAvatars[i] = createdAvatar` in both `CreateBotAvatar` and `InstantiateMatchAvatars`, check `_pendingRebuildIndices.Remove(i)` → if true, call `RebuildAvatar(i)` which re-reads the now-correct `match_Avatars` data. HashSet deduplicates multiple AVATARSYNCs during build → exactly one rebuild. Safe for all modes: PreConnect (GameManager null → condition fails), bots (no style change → styleChanged false), PlayFab/Hetzner PvP. | GameManager.cs, NetworkManager.cs |
| **Mar 3** | **Glow() NullRef guards for all 5 fighting styles + Projectile fix** — NullReferenceException spam in Glow()/IsEnemyInMeleeProximity() methods when remote player not fully initialized (character/stateManager null during AVATARSYNC processing or async avatar building). Stack: `CastManager.Update()` → `*Player.Glow()` → `Player.IsDead()` → `StateManager.GetState()` → NullRef. Fix: Added null guards before `IsDead()` calls: AmukthaPlayer/MukthaMukthaPlayer `IsEnemyInMeleeProximity()` (`if (targetPlayer.character == null \|\| targetPlayer.character.stateManager == null) return false`), MantraMukthaPlayer/YantramukthaPlayer `GlowIfInRange()` (`if (glowPlayer.character == null \|\| glowPlayer.playerObject == null) return`), PaniMukthaPlayer `Glow()` (`if (targetPlayer.character == null \|\| targetPlayer.character.stateManager == null) continue`). Also fixed Projectile.OnUpdate() SphereCast hit: added null checks for hitPlayer, hitPlayer.character, and hitPlayer.character.stateManager before accessing state. | AmukthaPlayer.cs, MukthaMukthaPlayer.cs, MantraMukthaPlayer.cs, PaniMukthaPlayer.cs, YantramukthaPlayer.cs, Projectile.cs |
| **Mar 3** | **Shield crack counter reset on deactivation** — Shield crack visuals could persist or appear immediately on next activation because `shieldHitCount` was not reset in `DeactivateShield()`. Fix: Added `shieldHitCount = 0` reset alongside existing `shieldActive = false`. | ShieldVFX.cs |
| **Mar 3** | **Shield crack contact point rotation + immediate crack appearance + Yantramuktha Pierce fix** — Shield cracks appeared underwater/off-shield with wrong rotation, only appeared after integer integrity thresholds, and Yantramuktha (bow/arrows) dealt 0.5 integrity instead of 0.0. Root causes: (1) `UpdateShieldIntegrity()` used camera-facing `LookAt()` instead of physical attack contact point. (2) Crack appearance used `Mathf.FloorToInt()` delaying first crack until 1.0 integrity lost. (3) Yantramuktha correctly mapped to `MeleeImpactType.Pierce` (0.0 integrity) but was never tested with updated attribute system. Fix: (1) Added `contactPointX/Y/Z` fields to `Packet.LogData` struct (+12 bytes, breaking change requires atomic server+client deployment). (2) Server extracts contact point from `PlayerBase.ContactPoint.transform.position` and includes in LogData packets at 4 locations in ElementalShield.cs (HandleAxeThrow, HandlePhysicalAttack, HandleElementalAttack Pierce + normal flow). Updated `Utils.SendLogData()` signature to accept Vector3 contactPoint. (3) Client `CombatLogHandler` extracts contact point from packet and passes to `UpdateShieldIntegrity()`. (4) `ShieldVFX.UpdateShieldIntegrity()` uses `LookAt(contactPoint)` for correct rotation (restores original HitShield() behavior). (5) Changed crack timing: Crack 1 appears on ANY damage (`integrityLost > 0`), Crack 2 at `lost >= 1.0`, Crack 3 at `lost >= 2.0`. (6) Added debug logging to server ElementalShield.HandlePhysicalAttack to verify ImpactType values. Impact: Cracks now face physical attack direction (sword/arrow impact point), first crack appears immediately when shield takes any damage, Yantramuktha correctly deals 0.0 shield integrity. Monotonic increase protection preserved (prevents race condition flicker). **Attribute system verified:** Pierce=0.00f, Slice=0.25f, Force=0.50f, ForceAndSlice=0.75f. | Client: Packet.cs, CombatLogHandler.cs, ShieldVFX.cs, ShieldManager.cs; Server: Packet.cs, Utils.cs, ElementalShield.cs, PlayerBase.cs |
...
- **Production:** Deployed to both DEV and PROD. CloudScript Dev rev 133, Prod rev 60.

### Lobby System (CRITICAL)
...
- **Fix:** `GameManager.RebuildAvatar(int avatarIndex)` — destroys old model, rebuilds from updated `match_Avatars` data. Triggered by `NetworkManager.ApplyAvatarSyncToMatchAvatars()` when fightingStyle, fighterClass, or wearItems change. Two paths: (1) model already built (`matchAvatars[i] != null`) → immediate `RebuildAvatar()`, (2) model mid-build (`matchAvatars[i] == null`) → `RequestDeferredRebuild(i)` queues into `_pendingRebuildIndices` HashSet → fires after `CreateBotAvatar`/`InstantiateMatchAvatars` stores the completed model.
...
*Last Updated: March 5, 2026 (Party/Lobby 2v2v2 map size fix — _activeMatchType pattern protects currentMatchType across scene transitions.)*
